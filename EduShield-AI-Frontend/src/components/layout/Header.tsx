...
import { Bell, Search, Calendar, AlertTriangle, Check, CheckCircle2, Eye, ShieldAlert, Loader2, Clock } from 'lucide-react';
...
export default function Header({ title, subtitle }: { title: string; subtitle?: string }) {
  ...
  const getFlagText = () => {
    ...
    if (unreadCount > 0) return `${unreadCount} Active`;
    return 'Stable';
  };
                ...
                >
                  <div className="px-4 py-3 border-b border-[#E8DDD0] flex items-center justify-between bg-[#FAF7F2]">
                    <h3 className="text-sm font-semibold text-[#1A1A2E] flex items-center gap-1.5">
                      <Bell className="w-4 h-4 text-[#C75B39]" /> Early Intervention Feed
                    </h3>
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y divide-[#E8DDD0]/40">
                    {alerts.length === 0 ? (
                      <div className="px-4 py-8 text-center text-gray-400 space-y-1">
                        ...
                        <p className="text-xs font-bold text-[#1A1A2E]">All Systems Stable</p>
                        <p className="text-[10px]">No unread risk alerts or overdue actions</p>
                      </div>
                    ) : (
                      alerts.map((notif) => (
                        <div
